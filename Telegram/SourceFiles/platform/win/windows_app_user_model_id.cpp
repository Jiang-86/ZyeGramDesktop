...
#ifdef OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"ZyeGram.ZyeGramDesktop.Store";
#else // OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"ZyeGram.ZyeGramDesktop";
#endif // OS_WIN_STORE
...
void CleanupShortcut() {
	...
	QString path = systemShortcutPath() + u"ZyeGram.lnk"_q;
...
bool checkInstalled(QString path = {}) {
	...
	const auto installed = u"ZyeGram Desktop/ZyeGram.lnk"_q;
	const auto oldInstalled = u"AyuGram Desktop/AyuGram.lnk"_q;
	const auto old = u"AyuGram for Windows/AyuGram.lnk"_q;
	return validateShortcutAt(path + installed)
		|| validateShortcutAt(path + oldInstalled)
